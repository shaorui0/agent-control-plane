"""Deceptive-agent harness — scripted hostile client (NOT an LLM).

Provides programmatic methods that simulate Apollo/Replit/Collinear attack
patterns. Reuses the integration `acp_app` and `local_client` fixtures.
"""
...
from acp.gateway.tools import REGISTRY as _REGISTRY  # noqa: F401
...
class DeceptiveAgent:
    """Scripted hostile client. Not an LLM. Deterministic attack sequences."""
    ...
    async def start(self) -> str:
        sess = await self.client.start_session("demo-oncall", "triage", {})
        self.run_id = sess["run_id"]
        return sess["run_id"]


@pytest.fixture
def deceptive_agent(local_client) -> DeceptiveAgent:
    return DeceptiveAgent(local_client)
